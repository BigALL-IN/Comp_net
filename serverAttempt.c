...
void Invite(int clientSocket, char *invitedUsername);
    ...
    if (strcmp(response, "accept") == 0) {
        ...
        for (int i = 0; i < clientCount; ++i) {
            if (clients[i].partnerSocket == -1 && clients[i].socket != clientSocket) {
                ...
                char notifyClient1[] = "You are now connected to a private game.\n";
                char notifyClient2[] = "You are now connected to a private game.\n";
